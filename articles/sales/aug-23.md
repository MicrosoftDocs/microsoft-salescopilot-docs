...
| CRM operations | Record type filter for polymorphic fields   | Sellers can now filter search results in polymorphic fields (fields which accept records of more than one type) other than the connected record.   | Outlook add-in | NA | Enabled by default | General availability |
