...
# Sales agent deployment guide for Salesforce CRM customers

Follow the instructions in this guide to deploy the Sales agent for your Salesforce CRM customers.
...
## Step 2: Create a setup policy to auto install and pin the Sales agent in Teams
...
The Sales agent applies your organization's existing CRM access controls and user permissions. Administrators must have correct permissions to customize their CRM systems, and users must have the correct permissions to view, update, and create records in their CRM systems from the Sales agent.

Salesforce administrators who need to customize the Sales agent [must have appropriate permissions](privileges.md#privileges-required-for-salesforce-customers).

Users of the Sales agent need to be API enabled in Salesforce so that they can access Salesforce using APIs. [Learn how to grant API Enabled permission](tsg-api-perm.md).
...
The Sales agent uses the Power Platform connector to connect to Salesforce CRM. Ensure that the connector is enabled for the Sales agent users.
...
The Sales agent automatically assigns all Power Platform administrators and Microsoft 365 global administrators to the System Administrator role in the trial environment. After the environment is created, it is recommended to review the list of administrators to ensure appropriate access levels.
...
## Step 8 (optional): Customize the Sales agent
...
## Step 9: Set up Sales agent in Microsoft 365 Copilot
...
## Step 10: Welcome sellers to the Sales agent
