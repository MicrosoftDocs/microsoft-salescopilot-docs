...
# Set up Lead Research and Outreach (preview)
...
Lead Research and Outreach connects to both Microsoft Dynamics 365 Sales and Salesforce Sales Cloud to research your leads, and discover related contact, account, opportunity, and activity records. Lead Research and Outreach uncovers the insights hidden in your CRM that go easily missed.
...
> [!NOTE]
> - Lead Research and Outreach relies on Dataverse Search indexes. Enabling this feature may increase your environment's Dataverse capacity consumption. [Know more about Dataverse search](/power-apps/user/relevance-search-benefits#what-actions-can-makers-or-admins-take-to-manage-dataverse-search-efficiently)
...
- Convert the **msdyn_viva** environment to a production environment while [setting up Lead Research and Outreach with Salesforce](#step-3-set-up-and-activate-the-agent).
- Configure the server-to-server connection between the Sales agent and Salesforce. Learn more about [Connect your agents to a data source](connect-agent-datasource.md)

## Step 1: Create a view and query that defines the leads to research

Lead Research and Outreach uses a CRM list view to find, filter, and monitor the records to research. The list view also defines which fields the agent has access to read. You must create a list view that's specific to the Lead Research and Outreach and make sure it has access to that list view.

Lead Research and Outreach monitors the leads that are included in the query results and researches each of the leads returned. It also monitors the set of leads for changes and runs research anytime a record is modified.
...
1. In the Sales agent admin settings, select **Sales Agent - Lead Research**.
1. If you're using a trial environment, convert it to a production environment:
    ...
    > [!CAUTION]
    > It may take several hours to convert a trial environment to a production environment. The Sales agent will be unavailable on all platforms during this process. You must start the conversion outside of business hours to minimize disruption. This action can't be undone. Production environments are billed for Microsoft Dataverse storage overages. Learn more: [Dataverse capacity-based storage details](https://go.microsoft.com/fwlink/?linkid=2332539)
...
1. In the **Map the fields** step, map the fields in your lead table to the fields used by Lead Research and Outreach, and then select **Next**. 
...
If the Lead Research and Outreach needs to access custom objects or custom fields configured in Salesforce, you need to give the Lead Research and Outreach explicit permission to read those objects and fields. For example, if you configured **Additional fields the agent should know about (recommended)**, the Lead Research and Outreach needs to be granted read access to each of those fields on your lead object.
...
If configured to write research summaries to Salesforce, you need to give the Lead Research and Outreach permission to write to the summaries table.
...
If configured to write research summaries to Salesforce, you need to give the Lead Research and Outreach permission to write to the lead records to link to their summary.
...
Once the Lead Research and Outreach is activated, you must [integrate Lead Research and Outreach with Salesforce](set-up-sales-agent-salesforce.md) to create a link to the lead's research, or optionally embed a widget to display the research summary on the lead page.
