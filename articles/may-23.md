...
| Core Experiences | Search improvements to look up any records by its name and key fields | Administrators can enable up to two key fields for OOB and custom entities. </br>Sellers will see the newly added key fields when search results are displayed in the Outlook side pane when connecting activities to CRM records or setting a related entity. | Outlook add-in | [Admin guide](customize-forms-and-fields.md#search-behavior-in-sales-copilot-add-in-for-outlook) | Enabled by admin | General availability |
