---
title: Use server-side synchronization with Sales agent
...
# Use server-side synchronization with Sales agent

> [!NOTE]
> This article is relevant only if you've connected the Sales agent to your Dynamics 365 environment.
...
- All participants in an email or appointments who are the Sales agent users will see the same status (saved or unsaved) of the Outlook activity.  
...
If you're a CRM administrator, you can simplify seller experience by setting up server-side synchronization of emails and appointments for all the Sales agent users. For information about enabling server-side synchronization, see [Connect to Exchange Online](/power-platform/admin/connect-exchange-online).  
If you're a seller and if server-side synchronization isn't enabled for you by your administrator, you're prompted to enable server-side synchronization for your mailbox when you save Outlook activities to Dynamics 365 using the Sales agent for the first time.
...
For a Sales agent user to save Outlook emails and appointments to Dynamics 365 from the Sales agent, their mailboxes need to be:
