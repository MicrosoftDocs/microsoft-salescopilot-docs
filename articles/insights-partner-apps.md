...
description: View insights from partner applications within the Sales agent.
ms.date: 11/20/2025
ms.topic: how-to
...
You can view records that are related to CRM records and activities that are related to CRM opportunities from partner applications within the Sales agent. To view related records and latest activities from partner applications, you must:
...
> - All Power Platform connectors are not certified to work with the Sales agent. The Sales agent displays activities from partner applications that have implemented specific APIs and made them available through their Power Platform connectors.
> - If you are a partner application maker and would like to integrate with the Sales agent, see [Extend the Sales agent](extend-sales-app.md). Currently, DocuSign can be integrated with the Sales agent. 
...
All the information about related records comes from the partner applications. The Sales agent renders the related record information retrieved from the partner application through the Power Platform connector. The Sales agent does not edit or filter the information.
...
All the information about activities comes from the partner applications. The Sales agent renders the activity information retrieved from the partner application through the Power Platform connector. The Sales agent does not edit or filter the information.
